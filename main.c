...
void hid_demo_task(void *pvParameters)
{
	...
	int8_t dy = 0;
      ...
      */
	if (up_down > 15.0f) {
		if (up_down > 50.0f) { dy = A_LOT_DOWN; }
		else { dy = A_BIT_DOWN; }
 	}
        else if (up_down < -15.0f) {
		if (up_down < -50.0f) { dy = A_LOT_UP; }
		else { dy = A_BIT_UP; }
	}
        if (left_right > 15.0f) {
		if (left_right > 50.0f) { dx = A_LOT_RIGHT; }
		else { dx = A_BIT_RIGHT; }
        }
	else if (left_right < -15.0f) {
		if (left_right < -50.0f) { dx = A_LOT_LEFT; }
		else { dx = A_BIT_LEFT; }
	}
	...
	*/
	...
	esp_hidd_send_mouse_value(hid_conn_id, 0x00, dx, dy);
	
	if (click == 30) { left_clk = true; }
	else { click++; }
	if (left_clk) {
		esp_hidd_send_mouse_value(hid_conn_id, 0x01, 0, 0);
